...
import cr.ac.una.sigeceunaemail.model.UserDto;
import cr.ac.una.sigeceunaemail.service.UserService;
import cr.ac.una.sigeceunaemail.util.AppContext;
import cr.ac.una.sigeceunaemail.util.FlowController;
import cr.ac.una.sigeceunaemail.util.Formato;
    ...
    private void validateUser() {
        try {
            if (txtUser.getText() == null || txtUser.getText().isBlank()) {
                ...
                Response responseTemp=userService.validateUserWithTemppasswordRs(this.txtUser.getText(),this.txtPassword.getText());
                if (response.getState()) {
                    UserDto userDto=(UserDto)response.getResult("User");
                    AppContext.getInstance().set("role", userDto.Role());
                    if(AppContext.getInstance().get("role")!="Sin asignar"){                
                    ...
                    else{new Message().showModal(Alert.AlertType.WARNING, "Access Denied", getStage(),
                            "Only admin and normal users are allowed to log in.");}
                }
                else if(responseTemp.getState()&&!response.getState()){
                    UserDto user=(UserDto)responseTemp.getResult("User");
                    this.txtPassword.clear();
                    this.txtUser.clear();
